...
func sqlxx() {
	...
	db, err := sqlx.Open("mysql", "root:@tcp(127.0.0.1:3306)/test?timeout=10s&readTimeout=10s&writeTimeout=10s")
	if err != nil {
		log.Fatalln(err)
	}
	...
	db.MustExec(persionSchema)
	db.MustExec(placeSchema)
	...
	//var places []Place
	//err = db.Select(&places, "SELECT * FROM place ORDER BY telcode ")
	//if err != nil {
	//	fmt.Println(err)
	//	return
	//}
	...
	rows, err := db.NamedQuery(`SELECT * FROM person WHERE first_name in(:first)`, map[string]interface{}{
		"first": []string{"abc", "def"},
	})
