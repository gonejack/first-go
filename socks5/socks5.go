...
func pipeWhenClose(client net.Conn, target string) {
	if Verbose {
		log.Printf("Connecting %s ", target)
	}

	server, err := net.DialTimeout("tcp", target, time.Second*15)
	if err != nil {
		log.Printf("Connect => %s:%s", target, err)
		return
	}
	defer server.Close()
	...
	if Verbose {
		log.Printf("Connect => %s:%d success", tcpAddr.IP, tcpAddr.Port)
	}
	...
	client.Write(rep[0 : pindex+2])

	// Transfer data
	go io.Copy(server, client) // client => server
	io.Copy(client, server)    // client <= server
}
...
func main() {
	...
	flag.BoolVar(&Verbose, "v", false, "should every proxy request be logged to stdout")
	addr := flag.String("addr", ":1086", "proxy listen address")
	flag.Parse()
